...
| ------------ | ------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `ts-gate/`   | `bash ts-gate/install.sh <project>`   | `ts-gate/` copied in, `eslint.config.mjs`, `biome.json` and `vitest.config.mjs` (tests never reach the network) when absent, a `Stop` hook, `.claude/rules/ts-*.md`, allow rules for the gate and the test runner, `workbench.premerge` |
