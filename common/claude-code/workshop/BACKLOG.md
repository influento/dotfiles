...
- turn caps: set `maxTurns:` on `wb-reviewer` and `wb-gate` at twice the p95 turn count the first ten transcripts show; the worker already treats a partial return as three holds
- external resources per item: a session records what it starts outside the tree (port, container, temp dir) in a ledger under `<git-common-dir>/workbench/resources/<id>`, `status` lists them, `archive` reaps what is left and says so. A dev-server picker (`ss -tlnp` matched to worktrees) is one view on the ledger. Design first: who writes the ledger (a hook on the tool call, or the session by hand) decides whether it can be trusted
