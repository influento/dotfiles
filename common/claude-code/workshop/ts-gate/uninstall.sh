...
if(Array.isArray(s.permissions?.allow)){
  const ours=new Set(["Bash(npm ci)","Bash(npm run gate)","Bash(npm run gate:local)","Bash(npm run gate:full)","Bash(npm run gate:fix)","Bash(npm test:*)","Bash(npx vitest:*)","Bash(npx jest:*)"]);
