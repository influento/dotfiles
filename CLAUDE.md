...
| `common/claude-code/project/workbench/bin/workbench`                     | `~/.local/bin/workbench` (the only file deployed out of `project/`, because it is what opts a project in) | all         |
...
| `project/`         | only `workbench/bin/workbench` → `~/.local/bin/` | The two tools installed into a project, whole: `workbench/` (CLI, its two skills, agents, commands, tests; rendered in by `workbench init`) and `ts-gate/` (the TypeScript Stop-hook gate; copied in by its `install.sh`). Never linked — see `common/claude-code/project/CLAUDE.md` |
