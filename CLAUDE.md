...
| **tmux**             | `common/tmux/tmux.conf.tpl`         | Terminal multiplexer: prefix key, panes, status bar (themed), attention glyphs and counts, popups (`prefix g` lazygit, `prefix t` scratch shell, `prefix e` scrollback in nvim)                                                                                                                                                                                                                                             |
...
| **claude-tmux**      | `common/scripts/claude-tmux`        | Claude Code hooks (global `settings.json`) → `tmux-attention` flags; records each pane's session so `tmux-attach` can `claude --resume` it after a reboot and re-flag what it was waiting on |
