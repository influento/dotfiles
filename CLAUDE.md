...
| `common/claude-code/project/workbench/bin/workbench`                     | `~/.local/bin/workbench` (with `stack`, the only files deployed out of `project/`, because they are what opts a project in) | all         |
| `common/claude-code/project/stack/bin/stack`                     | `~/.local/bin/stack` (the package registry under `project/stack/packages/` is read from here at `stack add` time) | all         |
...
| **tmux**             | `common/tmux/tmux.conf.tpl`         | Terminal multiplexer: prefix key, panes, status bar (themed), attention glyphs and counts, popups (`prefix g` lazygit, `prefix t` scratch shell, `prefix e` scrollback in nvim with `]]`/`[[` between prompts — `common/nvim/lua/scrollback.lua`), ssh windows named `host(ssh)` from the destination as typed (zsh preexec → pane option `@ssh_host` → `automatic-rename-format`)                                                                                                                                                                                                                                             |
...
| **setup-github**     | `common/scripts/setup-github`       | First-login setup: SSH key, GitHub auth, git identity, private git root (`private.root` in `~/.gitconfig.local`), remote switch                                                                                                                                                                                                                                                                                                                                                           |
| **git-private**      | `common/scripts/git-private`        | `git private`: create `<private.root>/<name>.git` over SSH, add it as `origin` (or `mirror` beside a GitHub origin), push. Driven by the `private-remote` skill |
| **tmux-attention**   | `common/scripts/tmux-attention`     | A pane flags what it needs (`needs-you`/`done`/`working`); tmux shows the glyph in the window list and on pane borders, counts on the status right, `prefix f` picker, `prefix o` jump, desktop notification for panes out of view. See `common/scripts/CLAUDE.md` |
| **tmux-overlay**     | `common/scripts/tmux-overlay`       | `prefix v`: an nvim popup per tmux session, kept alive in a hidden session; same key inside hides it, state survives. See `common/scripts/CLAUDE.md` |
| **claude-tmux**      | `common/scripts/claude-tmux`        | Claude Code hooks (global `settings.json`) → `tmux-attention` flags; names a claude window from its first prompt; records each pane's session so `tmux-attach` can `claude --resume` it after a reboot and re-flag what it was waiting on |
| **Claude Code**      | `common/claude-code/`               | Claude Code: global settings, permissions, custom skills, and the workbench and stack CLIs (see `common/claude-code/CLAUDE.md` and "Claude Code Skills" below; `write-a-skill` drafts new skills, `skill-creator` tests/benchmarks them — `skill-creator` eval scripts need PyYAML + a browser, so they only fully work on workstation). Bootstrapped via Anthropic's native installer on first `install.sh` run; self-updates thereafter |
...
| **tmux-attach**           | `workstation/scripts/tmux-attach`                            | Terminal entry point: restores the saved tmux state, then attaches. Replaces continuum's auto-restore, which skipped restores at random and corrupted window layouts                          |
...
| ------------------ | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `skills/`          | symlinked to `~/.claude/skills/` (global) | `caveman`, `codebase-design`, `debloat`, `deepen`, `densify`, `docx`, `frontend-design`, `grilling`, `human-like-text`, `indexer`, `pdf`, `private-remote`, `prototype`, `resolving-merge-conflicts`, `show-me`, `simple-english`, `skill-creator`, `wat`, `write-a-skill`, `writing-for-agents` |
| `project/`         | only `workbench/bin/workbench` and `stack/bin/stack` → `~/.local/bin/` | The tools installed into a project, whole: `workbench/` (CLI, its two skills, agents, commands, tests; rendered in by `workbench init`), `ts-gate/` (the TypeScript Stop-hook gate; copied in by its `install.sh`) and `stack/` (the registry of libraries, toolkits and tools a project chooses one at a time with `stack add`: a rule, skills from the registry or skills.sh, a dependency, a read-only subtree under `repos/` when the source is worth reading, a CLAUDE.md line). Never linked — see `common/claude-code/project/CLAUDE.md` |
