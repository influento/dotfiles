...
| `common/claude-code/workshop/stack/bin/stack`                     | `~/.local/bin/stack` (the package registry under `workshop/stack/packages/` is read from here at `stack add` time) | all         |
...
| `workshop/`         | only `workbench/bin/workbench` and `stack/bin/stack` → `~/.local/bin/` | The tools installed into a project, whole: `workbench/` (CLI, its two skills, agents, commands, tests; rendered in by `workbench init`), `ts-gate/` (the TypeScript Stop-hook gate; copied in by its `install.sh`) and `stack/` (the registry of libraries, toolkits and tools a project chooses one at a time with `stack add`: a rule, skills from the registry or skills.sh, a dependency, a read-only subtree under `repos/` when the source is worth reading, a CLAUDE.md line). Never linked — see `common/claude-code/workshop/CLAUDE.md` |
