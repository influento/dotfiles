...
# workbench is a self-contained tool under common/claude-code/, not a script in
# common/scripts/ — see the claude-code case in deploy_configs.
chmod +x "${DOTFILES_DIR}/common/claude-code/workbench/bin/workbench" 2>/dev/null || true
if [[ "$PROFILE" == "workstation" ]]; then
